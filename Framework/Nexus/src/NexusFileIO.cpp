...
static void getGroup(hid_t groupID,
...
getNexusAllEntries(const std::string &filename) {
  ...
  if (fileID < 0) {
    throw std::invalid_argument("ERROR: couldn't open hdf5 file " + filename +
                                "\n");
  }
...
std::string RelativePath(const std::string &input,
                         const std::string separator) {

  auto lastPosition = input.find_last_of(separator);

  std::string output;
  if (lastPosition == input.npos) {
    output = input;
  } else if (lastPosition == input.size() - 1) {
    output = "";
  } else {
    output = input.substr(lastPosition + 1);
  }
