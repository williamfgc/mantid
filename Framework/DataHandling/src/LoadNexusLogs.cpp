...
void LoadNexusLogs::exec() {
  ...
  //  const std::map<std::string, std::set<std::string>> allEntriesMask =
  //      Mantid::NeXus::getNexusAllEntries(filename);

  // used in case m_allEntries is nullptr
  std::map<std::string, std::set<std::string>> allEntriesLocal;
  const std::map<std::string, std::set<std::string>> *allEntries = nullptr;

  // TODO might be called from parent algorithms other than LoadEvenNexus
  if (m_allEntries == nullptr) {
    // std::cout << "Found nullptr ENTRIES\n";
    allEntriesLocal = Mantid::NeXus::getNexusAllEntries(filename);
    allEntries = &allEntriesLocal;
  } else {
    // std::cout << "Found non-nullptr ENTRIES\n";
    allEntries = m_allEntries;
  }
  ...
  // here refactor first level call (by key)
  auto lf_LoadLogsByClass = [&](const std::string &group_class,
    ...
    for (const std::string &entry : entries) {
      ...
      if (std::count(entry.begin(), entry.end(), '/') == 2) {
        if (isLog) {
          loadLogs(file, entry, group_class, workspace, *allEntries);
        } else {
  ...
  auto lf_LoadLogsByName = [&](const std::string &group_name) {
    for (auto itGroupClass = allEntries->begin();
      ...
      loadLogs(file, absoluteGroupName, group_class, workspace, *allEntries);
    }
  };
  ...
  if (workspace->mutableRun().hasProperty("proton_log")) {
    ...
    try {
      ...
      // TODO: replace this with allEntries
      ...
      if (itEventData != allEntries->end()) {
        ...
        for (const std::string &event : events) {
          ...
          const std::string eventEntry = Mantid::NeXus::RelativePath(event);
...
void LoadNexusLogs::loadLogs(
  ...
  auto lf_LoadByLogClass = [&](const std::string &logClass,
    ...
    for (auto it = itPrefixBegin;
         ...
         ++it) {
      // must be third level regex
      if (std::count(it->begin(), it->end(), '/') == 3) {
        if (isNxLog) {
          loadNXLog(file, *it, logClass, workspace, allEntries);
        } else {
          loadSELog(file, *it, workspace, allEntries);
        }
      }
    }
  };
  ...
  const std::string entry_name =
      Mantid::NeXus::RelativePath(absolute_entry_name);
